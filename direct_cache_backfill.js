...
async function directCacheBackfill() {
  console.log('Starting direct cache backfill for ALL incomplete entries...\n');
  
  try {
    // Get ALL incomplete cache entries using direct SQL
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      if (taxonomy.error) {
        ...
        // Update with complete taxonomy using direct SQL
        const sql = `
          UPDATE inaturalist_classification_cache 
          SET kingdom = ${escapeSqlString(taxonomy.kingdom)}, 
    ...
    if (successCount > 0) {
      ...
      console.log(`Upload processing should now show significantly improved API lookup success rates.`);
    }

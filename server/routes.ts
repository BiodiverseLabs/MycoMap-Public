...
async function updateSpeciesStatistics(uploadId?: number, progressTracker?: Map<number, any>) {
  ...
  // Classification cache backfill endpoint
  app.post('/api/classification/backfill', async (req, res) => {
    try {
      ...
      for (let i = 0; i < batchSize; i++) {
        ...
        // Fetch taxonomy from iNaturalist API
        try {
          ...
          if (response.ok) {
            ...
            // If no taxonomy found, mark as invalid
            if (!taxonomyFound) {
              await pool.query(`
          ...
          console.error(`API error for ${genus}:`, apiError.message);
          // Mark as invalid on API failure
          await pool.query(`
            INSERT INTO inaturalist_classification_cache (genus, kingdom, updated_at)
            VALUES ($1, 'INVALID', NOW())
            ON CONFLICT (genus) DO UPDATE SET
      ...
      res.json({
        ...
        batchSuccessful: successCount,
        totalProcessed: parseInt(stats.total_processed),
        totalSuccessful: parseInt(stats.successful),
        remaining: parseInt(stats.remaining),
        percentComplete: Math.round(parseInt(stats.total_processed) / 1087 * 100)
      });

...
(async () => {
  ...
  console.log('[DB] Checking database connectivity...');
  const isDbHealthy = await validateDatabaseConnection();
  
  if (!isDbHealthy) {
    console.warn('[DB] Database connection failed, but starting server anyway with degraded functionality');
  }

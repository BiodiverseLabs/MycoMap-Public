...
import { 
  users, observations, uploads, contributors, species, redlistAssessments, inaturalistData, inaturalistPlaces, mushroomObserverData,
  ...
  async getValidationData(params: {
    ...
    let sqlQuery = `
      SELECT 
        ...
        CASE 
          WHEN i.place_ids IS NOT NULL THEN 
            (SELECT string_agg(display_name, ', ') 
             FROM inaturalist_places ip 
             WHERE ip.place_id = ANY(i.place_ids) AND ip.place_type = 'state')
          ELSE NULL 
        END as "inatState",

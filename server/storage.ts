...
export interface IStorage {
  ...
  // Species-specific aggregated stats - OPTIMIZED
  getSpeciesStats(speciesName: string): Promise<{
    totalObservations: number;
    uniqueStates: number;
  ...
  // Record Index
  getRecordIndex(limit?: number, offset?: number, stateFirstsOnly?: boolean, recent?: boolean, state?: string, globalFirstsOnly?: boolean, startDate?: string, endDate?: string, species?: string, collector?: string): Promise<Array<{
    id: number;
    species: string;
    state: string;
    reportDate: string;
  ...
  getFitnessObservationDates(username: string): Promise<string[]>;
...
export class MemoryStorage implements IStorage {
  ...
  async getObservationMetrics(startDate?: string, endDate?: string, state?: string): Promise<{
    ...
    if (state) {
      filteredObservations = filteredObservations.filter(obs => obs.state === state);
    }
  ...
  // Stub implementation for species stats - MemoryStorage doesn't support this
